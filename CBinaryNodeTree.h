...
public:
    ...
    @note function is inherited by CBST and can be overwritten */
    virtual CBinaryNodeTree<ItemType>&
...
protected:
    ...
    std::shared_ptr<CBinaryNode<ItemType>> GetRootPtr() const;
   
	/** This function assigns an address to m_rootPtr
    @param rootPtr: A new address to assign to m_rootPtr
    @return Nothing. */
    void SetRootPtr(std::shared_ptr<CBinaryNode<ItemType>> rootPtr);
    ...
    @param subTreePtr: A pointer of CBinaryNodeTree type for the root of the
        tree.
    @return An int value. */
    int GetHeightHelper(std::shared_ptr<CBinaryNode<ItemType>>
        subTreePtr) const;

    /** This function calculates the number of nodes of the tree by providing
        the root/subtree root pointer.
    @param subTreePtr: A pointer of CBinaryNodeTree
        type for the root of the tree.
    @return An int value. */
    int GetNumberOfNodesHelper(std::shared_ptr<CBinaryNode<ItemType>>
        subTreePtr) const;
    ...
    /** Finds the minimum key in subTreePtr.
    @param subTreePtr: A pointer of CBinaryNodeTree
        type for the root of the tree.
    @return ItemType: Returns the minimum value key. */
    ItemType
    ...
    @param subTreePtr: A pointer of CBinaryNodeTree
        type for the root of the tree.
    @return CBinaryNode: Returns the maximum ItemType Node. node. */
    std::shared_ptr<CBinaryNode<ItemType>>
        FindMaxNode(std::shared_ptr<CBinaryNode<ItemType>> subTreePtr);

    /** Finds the maximum key in subTreePtr.
    @param subTreePtr: A pointer of CBinaryNodeTree
        type for the root of the tree.
    @return ItemType: Returns the maximum value key. */
    ItemType
    ...
    @param success: A bool reference to report
        success or failture of the removal.
    @return CBinaryNode: Returns the updated subTreePtr
        after adding the new node. */
    virtual std::shared_ptr<CBinaryNode<ItemType>>
        RemoveValue(std::shared_ptr<CBinaryNode<ItemType>>
            subTreePtr, const ItemType &target, bool &success) = 0;
   
    /** Recursively searches for target value in the tree by using a
        preorder traversal.
    @param treePtr: A pointer of CBinaryNodeTree type for the root of the tree.
    @param target: A const ItemType reference to find.
    @param success: A bool reference to report
        success or failture of the search.
    @return CBinaryNode: Returns target node pointer. */
    virtual std::shared_ptr<CBinaryNode<ItemType>>
        FindNode(std::shared_ptr<CBinaryNode<ItemType>>
            treePtr, const ItemType &target, bool &success) const = 0;
   
    /** Copies the tree rooted at oldTreeRootPtr and returns a pointer to
        the copy.
    @param oldTreeRootPtr: A pointer of CBinaryNodeTree
        type for the root of the tree.
    @return CBinaryNode: Returns the new tree pointer. */
    std::shared_ptr<CBinaryNode<ItemType>>
        CopyTree(const std::shared_ptr<CBinaryNode<ItemType>>
    ...
    @param subTreePtr: A pointer of CBinaryNodeTree
        type for the root of the tree.
    @return Nothing. */
    void DestroyTree(std::shared_ptr<CBinaryNode<ItemType>>
        subTreePtr);
    ...
    void Levelorder(void Visit(const ItemType &item),
        std::shared_ptr<CBinaryNode<ItemType>> treePtr, int level) const;
...
private:
    ...
    std::shared_ptr<CBinaryNode<ItemType>> m_rootPtr;
};
#include "CBinaryNodeTree.cpp"
